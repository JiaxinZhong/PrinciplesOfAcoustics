...
\begin{frame}{Exercises*}
	\begin{outline}
		\1 \emph{1.} 简述流体运动的两种描述方法，即Lagrange方法和Euler方法。
		\1 解答：
		\1 Lagrange方法和Euler方法都是描述流体运动的物理模型。
		\1 Lagrange方法的目标是跟踪流体中每个流体质点的运动状态，使用该流体
		质点的初始时刻（$t=0$）的坐标（$(a,b,c)$）来标记质点，
		记录这个质点任意时刻的所在位置（$\vb{R}=\vb{R}(a,b,c,t)$）及对应
		的物理量$f\ut{\ L}(a,b,c,t)$，其中物理量$f$可以是质点位移、速度、压强、温度
		等等。
		\1 Euler 方法的目标是描述空间中固定几何点（$\vb{r}=(x,y,z)$）的流体
		状态，记录任意时刻流经这个几何点的流体质点的物理量$f\ut{\ E}(x,y,z,t)$，所有
		空间几何点的某物理量求解出来后，构成整个空间场的场分布。
		\1 在$t$时刻流经Euler 方法中所述的位于$(x,y,z)$处的流点，可认为是初始时刻位于
		$(a_0,b_0,c_0)$的流点，那么2种方法的物理量有对应关系：$f\ut{\ E}(x,
		y,z,t) = f\ut{\ L}(a_0,b_0,c_0,t)$。
	\end{outline}
\end{frame}
...
\subsection{Sound Intensity}
\begin{frame}{Oblique Incidence}{Sound Intensity and Conservation of Energy}
	\begin{outline}
		\1 sound intensity: 
		$
		\vb{I}\st{i} = I\st{i} \vb{n}\st{i}\qc
...
\part{\titlestring}
\title{\titlestring}
\lecture{\titlestring}{radi}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\section{Monopole}
\begin{frame}{Monopole}
	\begin{outline}
		\1 a \emph{monopole} or \emph{point source}: a small \emph{pulsating
		sphere}
	\end{outline}
	
	\begin{figure}
		\centering
		\animategraphics[loop,controls,width=0.6\linewidth]{10}{gif/radi/monopoleFieldAni-}{0}{29}
		\footnote{Available at: https://en.wikibooks.org/wiki/Aeroacoustics/Acoustic\_Sources}
	\end{figure}
\end{frame}
...
\begin{frame}{Mechanical-Acoustical Coupled System}
	\begin{outline}
		\1 the \emph{reactive force} on the pulsating sphere caused by
		the sound field: $F\st{r} = -Z\st{r}u$
		\1 the \emph{harmonic exciting force}: $F=F\st{a} \exp[\mj(
		\omega t -kz)]$
		\1 dynamic equation on the surface of the sphere
		$$
		M\st{m} \dv{u}{t} = F+F\st{r} -R\st{m} u -K\st{m}\int u\dd{t}
		\imp 
		M\st{m} \dv{u}{t} +R\st{m} u +K\st{m}\int u\dd{t} +Z\st{r}u = F
		$$
		\1 because $u(t) \propto \exp(\mj\omega t)$
		$$
		\imp u = \frac{F}{Z\st{m}+Z\st{r}}
		\qc \qty[Z\st{m}+Z\st{r}
		=(R\st{m}+R\st{r})+\mj \qty(X\st{r}+\omega M\st{m} -
	\frac{1}{\omega C\st{m}})]
		$$
		\1 the \emph{effective mass}: $M\st{m}+X\st{r}/\omega$
		\1 the \emph{adnexed mass}: $M\st{r}= X\st{r}/\omega$
		\1 the time averaged total power
		$$\overline{W} = \frac12 (R\st{m}+R\st{r}) u\st{a}^2$$
	\end{outline}
\end{frame}

\begin{frame}{Radiation Properties}
	\begin{outline}
		\1 \emph{Case (1)}: small sphere, i.e. $ka\ll 1$
		$$
		\begin{cases}
			R\st{r} &\approx \rho_0c_0S_0 (ka)^2\\
			X\st{r} &\approx \rho_0 c_0 S_0 (ka)
		\end{cases}
		$$
		\1 the adnexed mass
		$$M\st{r} \approx \rho_0S_0 a = 3\qty(\frac{4}{3}\uppi a^3\rho_0)
		=3M_0$$
		\1 \emph{Case (2)}: big sphere, i.e. $ka \gg 1$
		$$\begin{cases}
			R\st{r} & \approx \rho_0c_0S_0\\
			X\st{r} & \approx 0
		\end{cases}
			$$
	\end{outline}
...
\begin{frame}{Radiation Impedance}
	\begin{figure}
		\centering
		\includegraphics[width=0.6\textwidth]{img/radi/radiationImpedancePulsatingSphere.jpg}
	\end{figure}
\end{frame}

\begin{frame}{Acoustical Radiation Power}
	\begin{outline}
		\1 acoustical radiation resistance
		$$
		\frac{R\st{r}}{\rho_0c_0S_0} =
		\frac{(ka)^2}{1+(ka)^2}
		\approx \begin{cases}
			(ka)^2\qc & (ka\ll 1)\\
			1\qc & (ka \gg 1)
		\end{cases}
		$$
		\1 radial sound intensity 
		$$
		I_r = \frac{1}{2} \Re (p^* v_r)
		=\frac{|p\st{a}|^2}{2\rho_0c_0} =
		\frac12 R\st{r} \frac{|u\st{a}|^2}{4\uppi r^2}
		$$
		\1 sound radiation power
		$$\overline{W\st{r}} = \frac12 \Re \qty[(-F\st{r})^* u]
		=\frac12 R\st{r} |u\st{a}|^2$$
		$$\imp \overline{W\st{r}} = 4\uppi r^2 I\st{r}$$
		\1 the most often used equation to compute sound radiation power
		$$\overline{W\st{r}} = \frac12 R\st{a} |Q\st{a}|^2  = 
		\frac{\rho \omega k}{8\uppi} |Q|^2
		\qty(R\st{a} = \frac{R\st{r}}{S_0^2}= \left. \frac{p}{v_r}\right|_{r=a})
		$$
	\end{outline}
\end{frame}
...
\begin{frame}{Dipole}
	\begin{outline}
		\1 a \emph{dipole} : two monopoles with opposite phase
	\end{outline}
	
	\begin{figure}
		\centering
		\animategraphics[loop,controls,width=0.6\linewidth]{10}{gif/radi/dipoleFieldAni-}{0}{29}
		\footnote{Available at: https://en.wikibooks.org/wiki/Aeroacoustics/Acoustic\_Sources}
	\end{figure}
\end{frame}
...
\begin{frame}{Dipole}
	...
	\begin{figure}
		\centering
		\includegraphics[width=0.5\textwidth]{img/radi/dipoleSketch.jpg}
	\end{figure}
\end{frame}
...
\begin{frame}{Dipole}
	\begin{outline}
		\1 $Q_+(t) =- Q_-(t) = Q(t)$
		\1 \emph{Assumption (1): small source} $ka\ll 1$
		\1 Total field
		$$\Phi(\vb{r},t) = Q_+(t)g(|\vb{r_+}|) + Q_-(t)g(|\vb{r_-}|)$$
		\1 \emph{Assumption (2): far field} $kr\gg 1$
		$$|\vb{r}_+ | = |\vb{r}-\frac12 \vb{d}| \approx r-\frac{1}{2}d \cos \theta
		\qc |\vb{r}_- | = |\vb{r}+\frac12 \vb{d}| \approx r+\frac{1}{2}d \cos \theta$$
		$$ \imp \Phi(\vb{r},t) \approx Q(t) g(r) \qty[2\mj \sin(\frac{kd}{2}
		\cos \theta)]$$
		\1 \emph{Assumption (3): small separation} $kd <1$
		$$\imp \Phi(\vb{r},t) \approx \mj kd \cos \theta Q(t)g(r)$$
		\1 directivity
		$$D(\theta) \equiv \frac{|\Phi(\theta)|}{|\Phi|\st{max}}
		= |\cos\theta|$$
	\end{outline}
\end{frame}
...
\begin{frame}{Dipole}
	...
	\begin{figure}
		\centering
		\includegraphics[width=0.5\textwidth]{img/radi/dipoleDirectivity.jpg}
	\end{figure}
\end{frame}
...
\begin{frame}{Dipole}
	\begin{outline}
		\1 pressure
		$$p(\vb{r},t) = \mj k\rho_0 c_0 \Phi(\vb{r},t) = 
		-k^2 d\rho_0c_0\cos\theta Q(t)g(r)$$
		\1 velocity
		$$v_r(\vb{r},t) = -\pdv{\Phi(\vb{r},t)}{r}
		=
		-k^2 d\cos\theta \qty(1+\frac{1}{\mj kr})Q(t)g(r)$$
		\1 sound intensity
		$$ I_r (\vb{r},t) = \frac12 \Re(p^* v_r)  
		= \frac{k^4d^2 \rho_0 c_0 \cos^2 \theta |Q|^2}{32\uppi^2 r^2}$$
		\1 sound power
		$$ W = \iint_S I_r \dd S = \iint I_r r^2 \sin \theta \dd{\theta}
		\dd{\phi} = \frac13 (kd)^2 \frac{\rho_0 \omega k}{8\uppi} |Q|^2$$
	\end{outline}

\end{frame}

\begin{frame}{Calculation of Sound Power}
	\begin{outline}
		\1 calculation of sound power
		$$W= \iint_S \vb{I} \vdot \dd {\vb{S}} = \frac12\Re \qty(
		\iint_S p^* \vb{v}\vdot \dd \vb{S}) = \frac{1}{2} \Re(p^* Q)$$
		\1 monopole (Assumption: $ka\ll 1$)
		$$ p = \mj \rho_0 \omega Q(t) g(r)$$
		$$\imp W\st{m} = \frac{\rho_0 \omega k}{8\uppi}$$
		\1 $N$ monopoles (Assumption: $ka\ll 1$)
		$$W = \sum_{n=1}^N \iint_{S_n} \vb{I}_n \vdot \dd{\vb{S}_n}
		=\frac12 \Re\qty(\sum_{n=1}^N p_n^* Q_n)$$
		\1 dipole (Assumption: $ka\ll 1$)
		$$ p = \mj \rho_0 \omega Q(t)\qty[g(r_+)+g_(r_-)]$$
		$$\imp W\st{d} = W_+ +W_- = 2[1-\sinc (kd)]W\st{m} \approx \frac{1}{3}
		(kd)^2 W\st{m}$$
	\end{outline}
\end{frame}
...
\section{Quadrupole}
\begin{frame}{Quadrupole}
	\begin{outline}
		\1 \emph{quadrupole}: two identical dipoles with opposite phase and 
		separated by a small distance
	\end{outline}
	
	\begin{figure}
		\centering
		\animategraphics[loop,controls,width=0.6\linewidth]{10}{gif/radi/quadrupole-}{0}{29}
		\footnote{Available at: https://en.wikibooks.org/wiki/Aeroacoustics/Acoustic\_Sources}
	\end{figure}
\end{frame}
\begin{frame}{Quadrupole}
	\begin{outline}
		\1 quadrupole: two identical dipoles with opposite phase and 
		separated by a small distance
		$$W_{1,+}=W_{2,+} = W\st{m} \qty[1-\sinc(kd) - \sinc(kl) + 
		\sinc(kl-kd)]$$
		$$W_{1,-}=W_{2,-} = W\st{m} \qty[1-\sinc(kd) - \sinc(kl) + 
		\sinc(kl+kd)]$$
		\1 total sound power
		$$W = \sum_{n=1}^2 (W_{n,+}+W_{n,-}) \approx \frac{1}{10} (kd)^4
		W\st{m} \quad \qty(kd \ll 1, l = d)$$	
	\end{outline}

	\begin{figure}
		\centering
		\includegraphics[width=0.4\textwidth]{img/radi/quadrupole.jpg}
	\end{figure}
\end{frame}
...
\begin{frame}{Radiation from a Baffled Circular Plane Piston}
	\begin{outline}
		\1 each point of the source can be treated as a pointlike source, 
		emitting spherical waves as given by
		$$p = \mj \rho\st{air} \omega Q(t)g(r)$$
		\1 simplest situation --- \emph{circular piston source}: uniformly
		excited across its circular face and to be enclosed in an infinite
		baffle such that acoustic energy is only radiated in the forward 
		direction
	\end{outline}
	\begin{figure}
		\centering
		\includegraphics[width=0.5\textwidth]{img/radi/circularPistonSource.jpg}
	\end{figure}
\end{frame}

\begin{frame}{Circular Piston Source}
	\begin{outline}
		\1 each infinitesimal source element $\vb{r}_0 = \rho(\cos
		\phi, \sin \phi, 0)$ on the piston produces a
		differential pressure $\dd p$ at a point of observation 
		$\vb{r} = r(\sin \theta, 0, \cos\theta)$ at a 
		distance of $h=|\vb{r}-\vb{r}_0|$
		$$\dd \Phi(\vb{r}) = G(\vb{r};\vb{r}_0) \vb{u} \vdot \dd \vb{S}$$
		\1 only the sound field on the surface $xOz$ is needed to compute for symmetry
		\1 the normal velocity on the surface
		$$ u\st{n} (x,y,t) = u\st{a}(x,y) \me^{\mj \omega t }
		\qc (S:\rho < a\qc z=0)$$
		\1 the total pressure $p$ at the point $(r,\theta)$ is the integral
		of $\dd p$ over the full radiator surface --- \emph{Rayleigh Integral}
		\begin{align*}
			\Phi(\vb{r}) &= \iint_S G(\vb{r};\vb{r}_0) u\st{a}(\vb{r}_0)
			\dd S = 2\iint_S g(\vb{r}; \vb{r}_0) u\st{a}(\vb{r}_0)\dd  S\\
			&=\frac{1}{2\uppi} \iint_S \frac{\me^{-\mj k |\vb{r}-\vb{r}_0|
		}}{|\vb{r}-\vb{r}_0|} u\st{a}(\vb{r}_0)\dd S
		\end{align*}
		\1 Piston source
		$$\Phi(\vb{r}) = 
		\frac{u\st{a}}{2\uppi} \iint_S \frac{\me^{-\mj k |\vb{r}-\vb{r}_0|
		}}{|\vb{r}-\vb{r}_0|} \dd S
		$$
	\end{outline}
\end{frame}
...
\begin{frame}{Far Field}
	\begin{outline}
		\1 \emph{Far field} or \emph{Fraunhofer limit} --- $r\gg a$
		\begin{align*}
		h &= \sqrt{r^2 + \rho^2 - 2r \rho \cos(\vb{r_0}, \vb{r})}
		 = r\sqrt{1-\frac{2\rho}{r}\cos (\vb{r_0},\vb{r})
		+\frac{\rho^2}{r^2}}\\
		&\approx r-\rho \cos(\vb{r_0},\vb{r}) = r-\rho \sin\theta \cos \phi
	\end{align*}
	$$\qty( \cos(\vb{r}_0,\vb{r}) = \frac{\vb{r}_0\vdot \vb{r}}{
	r_0 r} = \sin\theta\cos\phi)$$
	$$\imp \Phi(r,\theta) \approx 
	2u\st{a}g(r) \int_0^a \rho \dd\rho \int_0^{2\uppi} \me^{\mj k \rho \sin
	\theta \cos\phi }\dd\phi = 2Q\st{a} g(r) D(\theta)$$
	$$\qty(\int_0^{2\uppi} \me^{\mj x \cos\phi}\dd\phi = 2\uppi J_0(x)
	\qc 
	\int xJ_0(x)\dd x = xJ_1(x))$$
	\end{outline}
...
\begin{frame}{Jinc function}
	\begin{outline}
		\1 \emph{Jinc function} or \emph{Sombrero function}
		$$\jinc x = \somb x \equiv \frac{2J_1(x)}{x} \qc \jinc 0 = 1$$
	\end{outline}
	\begin{figure}
		\begin{subfigure}{0.45\textwidth}
			\centering
			\includegraphics[width = \textwidth]{img/radi/jincFunction2D.jpg}
		\end{subfigure}
		\begin{subfigure}{0.45\textwidth}
			\centering
			\includegraphics[width = \textwidth]{img/radi/sombreroFunction3D.png}
		\end{subfigure}
	\end{figure}
\end{frame}

\begin{frame}{Directivity}
	\begin{outline}
		$$|D(\theta)| = \qty|\frac{2J_1(ka\sin\theta)}{ka\sin\theta}|$$
		\1 the first zero $\theta_1$ of $\jinc x $ occurs at $ka\sin\theta
		=3.83$ --- a measure of the angular half width of the \emph{
		principle lobe} of the acoustic pressure
		$$\sin\theta_1 = \frac{3.83}{ka} = 0.61 \frac{\lambda}{a}$$
		\1 the first \emph{sidelobe} is included between $\theta_1$ and 
		$\theta_2$
		$$\sin \theta_2 = \frac{7.02}{ka} = 1.12 \frac{\lambda}{a}$$
	\end{outline}
	\begin{figure}
		\begin{subfigure}{0.4\textwidth}
			\centering
			\includegraphics[width=\textwidth]{img/radi/circularPistonSourceDirectivity1.jpg}
		\end{subfigure}
		\begin{subfigure}{0.4\textwidth}
			\centering
			\includegraphics[width=0.8\textwidth]{img/radi/circularPistonSourceDirectivity2.jpg}
		\end{subfigure}
	\end{figure}
\end{frame}
...
\begin{frame}{Finite Beams}
	\begin{outline}
		\1 \emph{Assumption (1)}: axisymmetric radiation. polar coordinates $(
		\rho, \theta, z)$
		$$
		\frac{1}{\rho}\pdv{\rho}\qty(\rho \pdv{p}{\rho}) + 
		\pdv[2]{p}{z} -\frac{1}{c_0^2}\pdv[2]{p}{t} = 0
		$$
		\1 transformations of coordinates --- retarded time $\tau = 
	\omega\qty(t-{z}/{c_0})$; $\xi = \rho /a$; $\sigma = z/R_0$
		\1 Rayleigh distance $R_0 = {\uppi a^2}/{\lambda}=\frac12ka^2$
		$$
			\pdv{\rho} = \frac{1}{a}\pdv{\xi}\qc
			\pdv{t} = \omega \pdv{\tau}\qc
			\pdv{z} = -k \pdv{\tau}+\frac{1}{R_0} \pdv{\sigma}
		$$
		$$
		4\pdv{p}{\sigma}{\tau} - \laplacian_\perp p - \frac{4}{(ka)^2}
		\pdv[2]{p}{\sigma}=0\qc \qty[\laplacian_\perp = \frac{1}{\xi}
		\pdv{\xi} \qty(\xi \pdv{\xi})]$$
		\1 \emph{Assumption (2)}: $ka\gg 1$
		$$
		4\pdv{p}{\sigma}{\tau} - \laplacian_\perp p =0
		$$
		\1 propagating waves: 
		$p(\tau, \sigma,\xi) = p(\sigma ,\xi) \me^{\mj \tau}$
		$$
		4\mj \pdv{p}{\sigma} - \laplacian_\perp p =0
		$$
	\end{outline}
\end{frame}

\begin{frame}{Hankel Transformations}
	\begin{outline}
		\1 \emph{Fourier-Bessel Integral Transformation}
		$$
		\begin{dcases}
			p(\sigma,\xi) & = \int_0^\infty q(\sigma,\eta)J_0(\xi \eta)
			\eta \dd{\eta},\\
			q(\sigma,\eta) & = \int_0^\infty p(\sigma, \xi)J_0(\xi,\eta)
			\xi \dd{\xi}
		\end{dcases}
		\quad
		\qty[\frac{1}{\xi} \pdv{\xi} \qty(\xi \pdv{\xi})J_0(\eta\xi)
		=-\eta^2 J_0(\eta\xi)]
		$$
		\1 the equation becomes
		$$4\mj \pdv{\sigma} q(\sigma, \eta)+\eta^2 q(\sigma,\eta)=0$$
		\1 solution
		$$q(\sigma,\eta) = q(0,\eta)\me^{\mj \eta^2\sigma/4}$$
		$$\imp \quad p(\sigma,\xi) = \int_0^\infty q(0,\eta) \me^{
		\mj \eta^2\sigma /4}J_0(\xi\eta)\eta \dd{\eta}$$
		\1 $ka \gg 1$
		$$p(\sigma,\xi) \approx \rho_0c_0 v(\sigma,\xi) 
		\xleftrightarrow{\text{Hankel Transform}} q(\sigma,\eta)\approx
	\rho_0c_0 w(\sigma,\eta)$$
	$$w(0,\eta) = \int_0^\infty u\st{a}(\xi)J_0(\xi\eta)\xi\dd \xi$$
	\end{outline}
\end{frame}

\begin{frame}{Sound Field Solution}
	\begin{outline}
		\1 solution
		$$p(\sigma,\xi) = \rho_0c_0 \int_0^\infty \zeta u\st{a} \dd\zeta
	\int_0^\infty \me^{\mj\eta^2\sigma/4}J_0(\xi\eta)J_0(\zeta \eta)\dd
\eta$$
		\1 Bessel integral relation
		$$\int_0^\infty \me^{-Q^2 x^2}J_0(\alpha x)J_0(\beta x)x \dd{x}
		=\frac{1}{2Q^2} \exp\qty(-\frac{\alpha^2+\beta^2}{4Q^2})
		J_0\qty(\mj \frac{\alpha\beta}{2Q^2})$$
		$$\imp \quad p(\sigma,\xi) = \rho_0c_0
		\int_0^\infty \frac{2\mj}{\sigma} u\st{a} (\zeta)\exp\qty(
		-\mj \frac{\xi^2+\zeta^2}{\sigma})J_0\qty(\frac{2\xi\zeta}{\sigma})
		\zeta \dd\zeta$$
	\end{outline}
\end{frame}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\setcounter{framenumber}{0}
\renewcommand{\titlestring}{Scattering of Sound}
\part{\titlestring}
\title{\titlestring}
\lecture{\titlestring}{scatter}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\begin{frame}{Point Sources Scattering by a Rigid Sphere}
	\begin{outline}
		\1 For a point source at $\vb{r}\st{s} = (r\st{s}, \theta\st{s},
		\phi\st{s})$ such that
		$$p\st{inc} = \mj \rho\st{air} \omega Q(t)g(R)$$
		\1 the incident field is (Bownman1969 p.369)
		\begin{align*}
			p\st{tot} = p\st{inc} +p\st{scat} &= \frac{\rho\st{air}\omega Q(t)}{4\uppi}
		\sum_{n=0}^\infty (2n+1)\qty[
		j_n(kr_<)-a_n' h_n(kr_<)] h_n(kr_>)\\
		&\times\sum_{m=0}^n \eps_m \frac{(n-m)!}{(n+m)!} P_n^m(\cos\theta\st{s})
		P_n^m(\cos\theta)\cos[m(\phi-\phi\st{s})]
	\end{align*}
	\begin{flalign*}
		\text{where}&& a_n' = \frac{j_n'(ka)}{h_n'(ka)}&&
	\end{flalign*}
	\end{outline}
\end{frame}
...
\begin{frame}{Point Sources Scattering by a Rigid Sphere}
	\begin{outline}
		\1 In particular --- $\vb{r}\st{s} = (r\st{s},0,0)$
		$$p\st{tot} = \frac{\rho\st{air}\omega Q(t)}{4\uppi}
		\sum_{n=0}^\infty (2n+1)P_n(\cos\theta)h_n(kr_>)
		\qty[j_n(kr_<)-a_n'h_n(kr_<)]$$
		\1 In the farfield --- $(r\to \infty)$
		\begin{align*}
			p\st{tot} &= \mj \rho\st{air}\omega Q(t) g(r)
		\sum_{n=0}^\infty (-\mj )^n (2n+1) P_n(\cos\theta)\qty[
		j_n(kr\st{s})-a_n'h_n(kr\st{s})]\\
		&= \frac{ \rho\st{air}\omega k}{4\uppi} Q(t) h_0(kr)
		\sum_{n=0}^\infty (-\mj )^n (2n+1) P_n(\cos\theta)\qty[
		j_n(kr\st{s})-a_n'h_n(kr\st{s})]
	\end{align*}
	\end{outline}
...
\begin{frame}
	\begin{figure}
		\centering
		\animategraphics[loop,controls,width=0.5\linewidth]{10}{gif/viscosities-}{0}{65}
...
\section{1 Dimensional Nonlinear Waves}
\begin{frame}{1 Dimensional Nonlinear Waves}
	\begin{outline}
		\1 Continuity equation and Eulerian equation in one dimension
		$$\begin{dcases}
			\rho \pdv{v}{t} +\rho v \pdv{v}{x} = -\pdv{P}{x}\\
			\pdv{\rho}{t} + \pdv{(\rho v)}{x} = 0
		\end{dcases}
		$$
		\1 \emph{assumption: simple waves} such that $\rho = \rho(v),
		P = P(v)$
		$$
		\begin{dcases}
			\pdv{v}{t}+\qty(v +\frac1\rho \dv{P}{v})\pdv{v}{x} & = 0\\
			\pdv{\rho}{t} + \qty(v+\rho\dv{v}{\rho})\pdv{\rho}{x} &=0
		\end{dcases}
		\imp
		\begin{dcases}
			-\qty(\pdv{v}{t})\Big/ \qty(\pdv{v}{x}) & = 
			v+\frac1\rho \qty(\dv{P}{v})\\
			-\qty(\pdv{\rho}{t})\Big/ \qty(\pdv{\rho}{x}) & = 
			v+\rho \qty(\dv{v}{\rho})
		\end{dcases}
		$$
		$$
		\qty(\pdv{x}{t})_v = \qty(\pdv{x}{t})_\rho 
		\imp
		\frac1\rho\qty(\dv{P}{v})=\rho\qty(\dv{v}{\rho})
		\imp
		\qty(\dv{v}{\rho})^2 = \frac{c^2}{\rho^2}
		$$
		$$
		\imp v = \pm \int\frac{c}{\rho} \dd\rho = \pm \int\frac{\dd P}{
		\rho c}$$
	\end{outline}
\end{frame}
...
\begin{frame}{1 Dimensional Nonlinear Waves}
	\begin{outline}
		$$\qty(\pdv{x}{t})_v = \qty(\pdv{x}{t})_\rho = v\pm c$$
		\1 integrating the above equation --- \emph{Riemann Solution}
		$$\imp x = (v\pm c)t +f(v)$$
		$$ v = F\qty[x-(v\pm c)t]$$
		\1 small amplitude
		$$v = F\qty[x\pm ct]$$
...
\begin{frame}{Relation Between $c$ and $v$}
	\begin{outline}
		\1 equation of state
		$$\frac{c^2}{c_0^2} = \qty(\frac{\rho}{\rho_0})^{\gamma -1}
		\imp c = c_0 \qty(\frac{\rho}{\rho_0})^{(\gamma-1)/2}$$
		\begin{align*}
			v & = \pm \int \frac{c}{\rho}\dd \rho = \pm c_0\int_{\rho_0}^
			\rho \qty(\frac{\rho}{\rho_0})^{(\gamma -1)/2} \frac{\dd\rho}{
			\rho}\\
			&=\pm \frac{2c_0}{\gamma-1}\qty[\qty(\frac{\rho}{\rho_0})^{
			(\gamma-1)/2}-1] = \pm\frac{2c_0}{\gamma-1}\qty(\frac{c}{
			c_0}-1)
		\end{align*}
		\1 ideal gas
		$$\imp c = c_0 \pm \frac{\gamma -1}{2} v$$
		\1 general fluids
		$$c = c_0 \pm \frac{B}{2A} v\qc \qty(\frac{B}{A} = \gamma-1)$$
	\end{outline}
\end{frame}
