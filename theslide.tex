...
\begin{frame}{Eulerian Method}{The Equation of Continuity}
	\begin{outline}
		\1 \emph{mass flux} is the rate of mass flow per unit area: 
		$\vb{j}\st{m} = \rho \vb{v}$
...
\begin{frame}{Sound Power, Sound Intensity}
	\begin{outline}
		...
		\1 \emph{sound energy flux (density)}: the rate of energy transfer per unit area 
		($\si{W \cdot m^{-2}}= \si{W \cdot m^{-2}\cdot s^{-1}}$)
		$$\vb{I}
		\equiv p\vb{v}$$
		\1 \emph{sound intensity}: time average of sound energy flux
		$$ \bar{\vb{I}} = \overline{p\vb{v}}$$
		\1 plane sound waves:
		$$\bar{\vb{I}} = \frac{1}{\rho_0c_0} \overline{p^2} \vb{n} = 
		\bar{\eps}c_0 \vb{n}$$
		\1 harmonic plane sound waves:
		$$\bar{\vb{I}} = \frac12 \Re(p^* \vb{v}) =
