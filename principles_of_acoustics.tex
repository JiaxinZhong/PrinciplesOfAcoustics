...
\begin{frame}{Eulerian Method}{The Equation of Continuity}
	\begin{outline}
		\1 \emph{mass flux} is the rate of mass flow per unit area: $\vb{j}\utm = \rho \vb{v}$
...
\begin{frame}{Eulerian Method}{Homework}
	\begin{outline}[enumerate]
			...
			$$
				\pdv{\rho}{t} +\div{\vb{j}\utm}=\rho q
			$$
	\end{outline}

\end{frame}

\begin{frame}{Eulerian Method}
	\begin{outline}
...
\begin{frame}{Equation of State}
	...
	\begin{exampleblock}{Equation of State for a Perfect Gas}
		$$P=\rho \mathcal{R} T\utK$$
		where $T\utK$ in kelvins (K) is the \emph{absolute temperature}, and $\mathcal{R}$ is the \emph{specific gas constant} and depends on the \emph{universal gas constant} and the \emph{molecular weight}. 
...
\begin{frame}{Equation of State}
	...
	\begin{exampleblock}{General Fluids Adiabat}
		...
		\begin{align*}
			...
			&=P_0 + \frac{1}{1!} A\rho_0 \delta + \frac{1}{2!} \rho_0^2 B \delta^2+\frac{1}{3!}\rho_0^3 C \delta^3+\cdots
		\end{align*}
		where $\delta = (\rho-\rho_0)/\rho_0$, $A=\qty(\pdv{P}{\rho})_{s,0}$, $B=\qty(\pdv[2]{P}{\rho})_{s,0}$, $C=\qty(\pdv[3]{P}{\rho})_{s,0}$.
	\end{exampleblock}
...
\begin{frame}{Disturbance Quantities}
	\begin{outline}
		\1 Under mechanical disturbances, the quantities are changed as: $$P_0\to P, \quad T_0\to T,\quad  \rho_0 \to \rho$$
			\2 \emph{pressure disturbance}: $p=\Delta{P} (=\dd{P}) = P-P_0$
			\2 \emph{density disturbance}: $\rho' = \Delta\rho (=\dd{\rho})= \rho-\rho_0 $
			\2 \emph{temperature disturbance}: $\tau=\Delta T (=\dd{T})=T-T_0$
			\2 \emph{velocity disturbance}: $\vb{v}'=\Delta \vb{v} (=\dd{\vb{v} }) = \vb{v} - \vb{v}_0$
...
\begin{frame}{Sound Pressure with Its Units}
	...
	\begin{outline}
		\1 \emph{effective sound pressure} (only for periodical or quasi-periodical acoustic waves: $$p\ute(x) = \sqrt{\frac1T\int_0^T p^2(\vb{r},t)\dd{t} }$$
...
\begin{frame}{A Special Wave Equation}
	\begin{outline}
		...
		$$\rho \dv{v}{t} = -\pdv{P}{r} \quad \rightarrow \quad \rho_0 \pdv{v}{t} \approx -\pdv{p}{r}$$
		\1 \emph{Eulerian equation:}
		$$\pdv{t} (\rho S \dd{r}) = (\rho vS)_r - (\rho v S)_{r+\dd{r}} = -\pdv{(\rho vS)}{r} \dd{r} 
		\quad \rightarrow \quad S\pdv{\rho}{t} \approx -\rho_0 \pdv{(vS)}{r}$$
		\1 \emph{equation of state:}
		$$P = P(\rho) \quad \rightarrow\quad  p \approx c_0^2 \rho'$$
		\1 \emph{linear wave equation:}
		$$\qty[\pdv[2]{p}{r}+\pdv{p}{r} \dv{(\ln S)}{r} ]- \frac{1}{c_0^2} \pdv[2]{p}{t}\approx 0$$
	\end{outline}
\end{frame}

\begin{frame}{Planar Wave}
	\begin{outline}
		\1 1D wave equation
		$$\qty(\pdv[2]{x} - \frac{1}{c_0^2} \pdv[2]{t}) p =0$$
		$$\pdv{p}{\xi}{\eta} = 0, \qty(\xi = c_0t-x, \eta=c_0t+x)$$
		$$\imp p = f(\xi)+g(\eta) = f(c_0t-x) +g(c_0t+x)$$
		...
		\1 \emph{wave front}: a curved surface where the physical quantites are the same
			$$\zeta = c_0t \mp \vb{n}\vdot \vb{r} = \text{const.}$$
			$$\dd \zeta = 0 \imp c_0 = \pdv{t} (\vb{n} \vdot \vb{r})$$
		\1 velocity 
		$$\vb{v} = -\frac{1}{\rho_0} \int \grad p \dd{t} =\frac{p}{\rho_0c_0}\vb{n}, \quad \qty(\grad p(\zeta) = \vb{n} \dv{p}{\zeta}, \int p(\zeta) \dd{t} = \frac{1}{c_0} \int p(\zeta)\dd{\zeta})$$
		\end{outline}
\end{frame}
...
\begin{frame}{Harmonic Plane Wave}
	\begin{outline}
		...
		$$p(\vb{r},t) = p(\vb{r})\me^{\mj \omega t}$$
	\end{outline}
	\begin{alertblock}{Linear Harmonic Plane Wave (Helmholtze) Equation}
		$$\qty(\laplacian +k^2) p(\vb{r}) = 0$$
		the \emph{wave number}: $k = \omega/c_0$
	\end{alertblock}
	\begin{outline}
		\1 1D wave equation
		$$ \qty(\dv[2]{x}+k^2)p(x) = 0, \imp p(x) =p\uta \me^{\mj kx}\qor p\uta\me^{-\mj kx}$$
		\1 propagating wave with frequency $\omega$
		$$p(x,t) = p\uta \me^{\mj (\omega t\mp kx)}$$
		$$p(\vb{r},t) = p\uta \me^{\mj (\omega t\mp \vb{k}\vdot\vb{r})}, \qty(\vb{k} = k \vb{n})$$
	\end{outline}
\end{frame}

\begin{frame}{Sound Energy}
	\begin{outline}
		\1 consider the fluid particle $\Delta V$
		\1 \emph{kinetic energy}: $$\Delta E\utk=\frac12 \rho_0 v^2\Delta V_0 , \quad \qty(v = |\vb{v}|)$$
		\1 \emph{potential energy}: the work done by sound pressure $p$
		$$\Delta E\utp = -\int_{\Delta V_0}^{\Delta V} p \dd(\Delta V)\approx \Delta V_0 \beta_{s,0} \int_0^p p\dd{p} = \frac12 \beta_{s,0} p^2 \Delta V_0$$
		$$\qty(\frac{\dd(\Delta V)}{\Delta V_0}   \approx - \frac{\dd{\rho'}}{\rho_0} \xrightarrow{p=c_0^2\rho'} \dd(\Delta V) \approx -\Delta V_0 \beta_{s,0}\dd{p})$$
		\1 \emph{total energy}: $$\Delta E = \Delta E\utk+\Delta E\utp = \frac12 \qty(\rho_0 v^2+\beta_{s,0}p^2)\Delta V_0$$
	\end{outline}
\end{frame}
...
\begin{frame}{Oblique Incidence}{reflection and transmission coefficients}
	\begin{outline}
		...
		\begin{cases}
			...
			\displaystyle \frac{p_\text{ia} - p\utra}{z_\text{n1}} = \frac{p\utta}{z_\text{n2}}
		\end{cases}	
		\imp 
		\begin{cases}
			1+r\utp=t\utp\\
			\displaystyle \frac{1-r\utp}{z_\text{n1}} = \frac{t\utp}{z_\text{n2}}
		\end{cases}	
		,
		\qty(
		\begin{matrix}
			z_\text{n1}\equiv z\utni=-z\utnr\\
			z_\text{n2}\equiv z\utnt
		\end{matrix}	
		)
		$$
		...
		$$
		r\utp\equiv\frac{p\utra}{p\utia} = \frac{z_\text{n2}-z_\text{n1}}{z_\text{n2}-z_\text{n1}} = \frac{R_2\cos\theta\uti-R_1\cos\theta\utt}{R_2\cos\theta\uti+R_1\cos\theta\utt}=\frac{m\cos\theta\uti-\sqrt{n^2-\sin^2\theta\uti}}{m\cos\theta\uti+\sqrt{n^2-\sin^2\theta\uti}}
		$$
		$$
		t\utp\equiv\frac{p\utta}{p\utia} = \frac{2z_\text{n2}}{z_\text{n2}-z_\text{n1}} = \frac{2R_2\cos\theta\uti}{R_2\cos\theta\uti+R_1\cos\theta\utt}=\frac{2m\cos\theta\uti}{m\cos\theta\uti+\sqrt{n^2-\sin^2\theta\uti}}
		$$
	\end{outline}
\end{frame}
...
\begin{frame}{Radiation from a Baffled Circular Plane Piston}
	\begin{outline}
		...
		$$\Phi(\vb{r}) = \iint_{S_\text{pis}} 2g(\vb{r};\vb{r}_0) u\uta (\vb{r}_0) \dd{S_0} \xlongequal{u\uta =\text{const}} u\uta\iint_{S_\text{pis}} \frac{\me^{-\mj kh}}{2\uppi h}\dd{S_0}$$
	\end{outline}
...
\begin{frame}{Radiation from a Baffled Circular Plane Piston}
	\begin{exampleblock}{Sound Field at Far Field (Radiation from a Baffled Circular Plane Piston) }
		...
		\begin{flalign*}
			\text{where}&& Q(t) = Q\uta\me^{\mj \omega t},\quad  Q\uta = u\uta\qty( \uppi a^2),\quad g(r) \equiv \frac{\me^{-\mj kr}}{4\uppi r},\quad D(\theta) \equiv \frac{2\upJ_1(ka\sin\theta)}{ka\sin\theta}  & &
		\end{flalign*}
	\end{exampleblock}
\end{frame}
